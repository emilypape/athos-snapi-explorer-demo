...
- **Query preset** - a toggleable `Query: "wear"` chip, kept separate from the filter presets (filters combine freely; a query baked into each one would get silently overwritten by the next filter clicked). Also means a bare filter chip can run standalone on the autocomplete tab, which otherwise requires a non-empty query.
