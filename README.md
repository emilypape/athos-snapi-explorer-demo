...
- **Query preset** - a toggleable `Query: "tops"` chip that also sets a paired `subQuery` of `"hoodie"` (the "rq" refinement-query param, narrowing within the main query). Kept separate from the filter presets (filters combine freely; a query baked into each one would get silently overwritten by the next filter clicked). Also means a bare filter chip can run standalone on the autocomplete tab, which otherwise requires a non-empty query.
